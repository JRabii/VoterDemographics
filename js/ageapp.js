...
// Create function to find Min and Max values in ageData
  
function minimaxX(ageDataX) {
    xMin = d3.min(ageData, function (d) { return d[ageDataX] });
    xMax = d3.max(ageData, function (d) { return d[ageDataX] });
};

function minimaxY(ageDataY) {
    yMin = d3.min(ageData, function (d) { return d[ageDataY] });
    yMax = d3.max(ageData, function (d) { return d[ageDataY] });
};
...
.attr("opacity", 0.9)
    ...
    .html(function (d) {  
    return (`<strong>Age Range:</strong> ${d.ageRange}
